...
class _HomePageState extends State<HomePage> {
        ...
        physics: ScrollPhysics(),
          child: Column(
            children: <Widget>[
              ...
              stream: FirebaseFirestore.instance.collection('items').where('total_qty', isGreaterThan: 0).snapshots(),
              builder: (BuildContext context, AsyncSnapshot<QuerySnapshot> snapshot) {
                if (snapshot.hasData && snapshot.data!.docs.length > 0 ) {
                  _foodItems = [];
                  snapshot.data?.docs.forEach((item) {
                    _foodItems.add(Food(item.id , item['item_name'], item['total_qty'], item['price']));
                  });
                    ...
                    return Container(
                      ...
                      child:ListView.builder(
                        ...
                        return ListTile(
                          ...
                          trailing: IconButton(
                            icon: cartIds.contains(_suggestionList[i].id)? Icon(Icons.remove):Icon(Icons.add),
                            onPressed: () async{
  ...
  void getCart(String uuid) async{
    List<String> ids = [];
    QuerySnapshot snapshot = await FirebaseFirestore.instance.collection('carts').doc(uuid).collection('items').get();
