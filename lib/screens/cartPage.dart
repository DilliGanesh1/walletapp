...
class _CartPageState extends State<CartPage> {
        ...
        child: Column(
          children: <Widget>[
            StreamBuilder<QuerySnapshot>(
              stream: FirebaseFirestore.instance.collection('carts').doc(authNotifier.userDetails.uuid).collection('items').snapshots(),
  ...
  Widget dataDisplay(BuildContext context, String uid, List<String> foodIds, Map<String, int> count){
    return StreamBuilder<QuerySnapshot>(
      stream: FirebaseFirestore.instance.collection('items').where(FieldPath.documentId, whereIn: foodIds).snapshots(),
      builder: (BuildContext context, AsyncSnapshot<QuerySnapshot> snapshot) {
        if (snapshot.hasData && snapshot.data!.docs.length > 0 ) {
          List<Cart> _cartItems = [];
          snapshot.data!.docs.forEach((item) {
            _cartItems.add(Cart(item.id, count[item.id]!, item['item_name'], item['total_qty'], item['price']));
          });
