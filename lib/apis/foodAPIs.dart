...
forgotPassword(AppUser user, AuthNotifier authNotifier, BuildContext context) async {
  pr = new ProgressDialog(context, type: ProgressDialogType.normal, isDismissible: false, showLogs: false);
  pr.show();
  try {
...
addToCart(Food food, BuildContext context) async {
  ...
  try {
    ...
    QuerySnapshot data = await cartRef.doc(currentUser.uid).collection('items').get();
    ...
    await cartRef
      .doc(currentUser.uid).collection('items').doc(food.id).set({"count": 1})
      .catchError((e) => print(e))
...
removeFromCart(Food food, BuildContext context) async {
  ...
  try {
    ...
    CollectionReference cartRef = FirebaseFirestore.instance.collection('carts');
    await cartRef
      .doc(currentUser.uid).collection('items').doc(food.id).delete()
      .catchError((e) => print(e))
      .then((value) => print("Success"));
  } catch (error) {
    pr.hide().then((isHidden) {
...
addNewItem(String itemName, int price, int totalQty, BuildContext context) async {
  ...
  try {
    ...
    await itemRef
      .doc().set({"item_name": itemName, "price": price, "total_qty": totalQty})
      .catchError((e) => print(e))
      .then((value) => print("Success"));
  } catch (error) {
    pr.hide().then((isHidden) {
...
editItem(String itemName, int price, int totalQty, BuildContext context, String id) async {
  ...
  try {
    CollectionReference itemRef = FirebaseFirestore.instance.collection('items');
    await itemRef
      .doc(id).set({"item_name": itemName, "price": price, "total_qty": totalQty})
      .catchError((e) => print(e))
      .then((value) => print("Success"));
  } catch (error) {
    pr.hide().then((isHidden) {
...
editCartItem(String itemId, int count, BuildContext context) async {
  ...
  try {
    ...
    if(count <= 0){
      await cartRef
      .doc(currentUser.uid).collection('items').doc(itemId).delete()
      .catchError((e) => print(e))
      .then((value) => print("Success"));
    }else{
      await cartRef
        .doc(currentUser.uid).collection('items').doc(itemId).update({"count": count})
        .catchError((e) => print(e))
        .then((value) => print("Success"));
    }
  } catch (error) {
...
addMoney(int amount, BuildContext context, String id) async {
  ...
  try {
    ...
    await userRef
      .doc(id).update({'balance': FieldValue.increment(amount)})
      .catchError((e) => print(e))
      .then((value) => print("Success"));
  } catch (error) {
    pr.hide().then((isHidden) {
